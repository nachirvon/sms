jQuery(function ($) {
    ...
    completeRegBtn.on('click', function(){
        ...
        const fname = $('#otp_fname').val().trim();
        const lname = $('#otp_lname').val().trim();
        ...
        // اعتبارسنجی طول در سمت کلاینت
        if (fname.length > 50 || lname.length > 50) {
            showMessage('نام و نام خانوادگی نمی‌تواند بیشتر از ۵۰ کاراکتر باشد.', 'error');
            return;
        }
